...
class videomt(nn.Module):
    ...
    def from_config(cls, cfg):
        ...
        if deep_supervision:
            dec_layers = cfg.MODEL.BACKBONE.NUM_BLOCKS
            aux_weight_dict = {}
        ...
        return {
            "dynamic_eval" : cfg.MODEL.BACKBONE.DYNAMIC_EVAL,
            "img_size": cfg.MODEL.BACKBONE.IMG_SIZE,
            "backbone": backbone,
            # "sem_seg_head": sem_seg_head,
            "criterion": criterion,
            "num_queries": cfg.MODEL.BACKBONE.NUM_OBJECT_QUERIES,
            "object_mask_threshold": cfg.MODEL.BACKBONE.TEST.OBJECT_MASK_THRESHOLD,
            "overlap_threshold": cfg.MODEL.BACKBONE.TEST.OVERLAP_THRESHOLD,
            "metadata": MetadataCatalog.get(cfg.DATASETS.TRAIN[0]),
            "size_divisibility": cfg.MODEL.BACKBONE.SIZE_DIVISIBILITY,
...
class videomt_segmenter(videomt):
    """
    Training segmentation model (Mask2Former) with context-aware instance embeddings.
    """
    ...
    def from_config(cls, cfg):
        ...
        if deep_supervision:
            dec_layers = cfg.MODEL.MODEL.NUM_BLOCKS
            aux_weight_dict = {}
            for i in range(dec_layers - 1):
                aux_weight_dict.update({k + f"_{i}": v for k, v in weight_dict.items()})
            weight_dict.update(aux_weight_dict)
    ...
    def get_prototypical_contrastive_loss(self, outputs, targets):
        contrastive_items = []
        
        mask_features = outputs['mask_features']
        _, C, H, W = mask_features.shape
        B = len(targets)
        mask_features = mask_features.view(B, -1, C, H, W)
        for batch, targets_per_video in enumerate(targets):
            feat = mask_features[batch]  # (T, C, H, W)
            masks = targets_per_video['masks'].clone().detach()  # (N, T, H', W')
            num_instance = masks.shape[0]
            if masks.numel() > 0 and num_instance > 1:
                if masks.sum(dim=(1, 2, 3)).min() == 0:
                    continue
                masks = F.interpolate(masks, size=(H, W), mode='bilinear', align_corners=False)

                # Extract frame instance prototypes (See Eq.(11) in Sec. 4.2)
                prototypes = {k: [] for k in range(num_instance)}
                for frame_i in range(feat.shape[0]):
                    frame_feat = feat[frame_i]  # (C, H, W)
                    for instance_i in range(num_instance):
                        frame_mask = masks[instance_i, frame_i]  # (H, W)
                        if frame_mask.sum() > 0:
                            frame_prototype = frame_feat * frame_mask.unsqueeze(0)
                            frame_prototype = frame_prototype.sum(dim=(1, 2)) / frame_mask.sum()
                            prototypes[instance_i].append(frame_prototype)
                
                # Prepare contrastive learning pairs
                for instance_i in range(num_instance):
                    if not prototypes[instance_i]:
                        continue
                    pos_embed = torch.stack(prototypes[instance_i], dim=0)  # (T', C)
                    anchor = pos_embed.clone()
                    neg_embeds = [torch.stack(prototypes[i], dim=0) for i in range(num_instance) if i != instance_i and prototypes[i]]
                    if len(neg_embeds) == 0:
                        continue
                ...
                continue
        losses = loss_reid(contrastive_items, outputs['mask_features'], loss_name='loss_proto')
        
        return losses

    def post_processing(self, outputs):
        pred_logits, pred_masks, pred_embds = outputs['pred_logits'], outputs['pred_masks'], outputs['pred_embds']
        # print(pred_logits.shape, pred_masks.shape, pred_embds.shape)
        pred_logits = pred_logits[0]
        pred_masks = einops.rearrange(pred_masks[0], 'q t h w -> t q h w')
        pred_embds = einops.rearrange(pred_embds[0], 'c t q -> t q c')
    ...
    def inference_video_vis(
        ...
        if len(pred_cls) > 0:
            ...
            if aux_pred_cls is not None:
                print(f"aux_pred_cls: {aux_pred_cls}")	
...
@META_ARCH_REGISTRY.register()
class videomt_online(videomt_segmenter):
    """
    Training context-aware instance tracker.
    See Sec. 4.1.2.
    """
    ...
    def from_config(cls, cfg):
        ...
        weight_dict = {
            ...
            "loss_mask": mask_weight,
            "loss_dice": dice_weight,
            "loss_ce_extra": class_weight,
            "loss_mask_extra": mask_weight,
            "loss_dice_extra": dice_weight,
        }
        ...
        tracker = CAVIS_Tracker(
            hidden_channel=cfg.MODEL.BACKBONE.HIDDEN_DIM,
            feedforward_channel=cfg.MODEL.BACKBONE.DIM_FEEDFORWARD,
            num_head=cfg.MODEL.BACKBONE.NHEADS,
            decoder_layer_num=cfg.MODEL.TRACKER.DECODER_LAYERS,
            mask_dim=cfg.MODEL.BACKBONE.HIDDEN_DIM,
            class_num=cfg.MODEL.BACKBONE.NUM_CLASSES,
            backbone_dim = cfg.MODEL.BACKBONE.HIDDEN_DIM,
        )
        ...
        return {
            ...
            "window_size": cfg.MODEL.BACKBONE.TEST.WINDOW_SIZE,
            "task": cfg.MODEL.BACKBONE.TEST.TASK,
            "use_cl": cfg.MODEL.TRACKER.USE_CL,
        }
    ...
    def forward(self, batched_inputs):
        ...
        if not self.training and self.window_inference:
            ...
            # with torch.no_grad():
            image_outputs = self.backbone(images.tensor)
            ...
            frame_embds_no_norm = None
            mask_features = image_outputs['mask_features'].unsqueeze(0)
            del image_outputs['mask_features'] #, image_outputs['pred_reid_embed']
        ...
        if self.training:
            ...
            # Compute extra loss just from image_outputs
            extra_losses = self.criterion(image_outputs, targets)
            # Merge extra_losses without overwriting existing keys
            

            for k, v in extra_losses.items():
                if k in losses:
                    # If key exists in losses, rename it with "_extra"
                    match = re.match(r"(.+?)_(\d+)$", k)
                    if match:
                        base, num = match.groups()
                        new_key = f"{base}_extra_{num}"
                    else:
                        new_key = k + "_extra"
                    losses[new_key] = v
                else:
    ...
    def frame_decoder_loss_reshape(self, outputs, targets, image_outputs=None):
        ...
        if 'aux_outputs' in image_outputs:
            for i in range(len(image_outputs['aux_outputs'])):
                image_outputs['aux_outputs'][i]['pred_masks'] = einops.rearrange(
                    image_outputs['aux_outputs'][i]['pred_masks'], 'b q t h w -> (b t) q () h w'
                )
                image_outputs['aux_outputs'][i]['pred_logits'] = einops.rearrange(
                    image_outputs['aux_outputs'][i]['pred_logits'], 'b t q c -> (b t) q c'
                )
        if 'aux_outputs' in outputs:
            for i in range(len(outputs['aux_outputs'])):
                outputs['aux_outputs'][i]['pred_masks'] = einops.rearrange(
    ...
    def reset_image_output_order(self, output, indices):
        ...
        if 'aux_outputs' in output:
            for aux in output['aux_outputs']:
                aux['pred_logits'][0] = aux['pred_logits'][0][frame_indices, indices]
                aux['pred_masks'][0] = aux['pred_masks'][0][indices, frame_indices].transpose(0, 1)
    ...
    def run_window_inference(self, images_tensor, window_size=30):
        ...
        for i in range(iters):
            ...
            # del out['res2'], features['res3'], features['res4'], features['res5']
            for j in range(len(out['aux_outputs'])):
                del out['aux_outputs'][j]['pred_masks'], out['aux_outputs'][j]['pred_logits']
            # del out['pred_reid_embed']
            # referring tracker inference
            frame_embds = out['pred_embds']  # (b, c, t, q)
            frame_embds_no_norm = None #out['pred_embds_without_norm']
