...
class Decoder(nn.Module):
    ...
    def forward(
        ...
        attn_mask, lateral_feature = None, None
        mask_logits_per_layer, class_logits_per_layer = [], []

        # Process through decoder blocks with masking
        for i, block in enumerate(self.blocks):
            # Generate predictions and attention mask
            if self.masked_attn_enabled:
                mask_logits, class_logits, _ = self._predict(self.decoder_norm(x), H, W)
                mask_logits_per_layer.append(mask_logits)
                class_logits_per_layer.append(class_logits)

                attn_mask = self._attn_mask(x, mask_logits, i, H, W)
                
            x = self.block_forward(x, block, attn_mask, pe)

        # Final predictions
        mask_logits, class_logits, queries = self._predict(self.decoder_norm(x), H, W)
        mask_logits_per_layer.append(mask_logits)
        class_logits_per_layer.append(class_logits)

        # expand BT to B, T
        bt = mask_logits_per_layer[-1].shape[0]
        bs = bt // self.num_frames if self.training else 1
        t = bt // bs
        # print(bt, bs, t,self.num_frames,self.training)
        for i in range(len(mask_logits_per_layer)):
            mask_logits_per_layer[i] = einops.rearrange(mask_logits_per_layer[i], '(b t) q h w -> b q t h w', t=t)

        for i in range(len(class_logits_per_layer)):
            class_logits_per_layer[i] = einops.rearrange(class_logits_per_layer[i], '(b t) q c -> b t q c', t=t)

        
        
        pred_embds = einops.rearrange(queries, '(b t) q c -> b c t q', t=t)
       
        out = {
            'pred_logits': class_logits_per_layer[-1],
            'pred_masks': mask_logits_per_layer[-1],
            'aux_outputs': self._set_aux_loss(
                class_logits_per_layer if self.mask_classification else None, mask_logits_per_layer
            ),
